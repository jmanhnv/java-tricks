...
public class GroupByFeatureInJava {
	public static void main(String[] args) {
		...
		for (Iterator<ProductBean> iterator = list.iterator(); iterator.hasNext();) {
...
class ProductBean {
	...
	class CompareByProductID implements Comparator<ProductBean> {
		public int compare(ProductBean p1, ProductBean p2) {
			if (p1.productId.compareTo(p2.productId) > 0) { return 1; }
			if (p1.productId.compareTo(p2.productId) < 0) { return -1; }
	...
	class CompareByCreateDate implements Comparator<ProductBean> {
		public int compare(ProductBean p1, ProductBean p2) {
			if (p1.date.after(p2.date)) { return 1; }
			if (p1.date.before(p2.date)) { return -1; }
			return 0;
		}
